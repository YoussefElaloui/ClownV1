...
namespace exo1.Controllers
{
    public class ProductsController : Controller
    {
        ...
        // POST: Products
        [HttpPost]
        public ActionResult Index(string selectedCat)
        {
            IQueryable<Product> products = db.Products;
            if (selectedCat != "all")
            {
                int sscc = Convert.ToInt32(selectedCat);
                products = products.Where(r => r.CategoryID==sscc);
            }
            return PartialView("~/Views/Products/Prajax.cshtml",products.ToList());
        }
